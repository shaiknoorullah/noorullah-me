/* The film grade stack (DESIGN.md §4.4): SMAA -> god rays (high tier) ->
   DOF -> bloom -> CA -> per-act parametric grade -> grain -> vignette ->
   AgX tone mapping.

   All effects are instantiated directly and driven imperatively in
...
import { GradeEffect } from '../../lib/three/effects';
...
export function Effects({ director, sun }: { director: Director; sun: THREE.Mesh | null }) {
  ...
  const fx = useMemo(() => {
    ...
    const dof = low
      ...
      : new DepthOfFieldEffect(camera as THREE.PerspectiveCamera, {
          worldFocusDistance: 7.7,
          worldFocusRange: 14,
          bokehScale: 1.7, // a fast 35 wide open, not an anamorphic fantasy
          height: 480,
        });
  ...
  useFrame(() => {
    ...
    if (fx.dof) {
      ...
      // focusRange — 14m/aperture keeps a generous sharp field with
      // believable falloff at 35mm
